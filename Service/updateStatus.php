...
if (isset($data['ride_id']) && isset($data['ride_status']) && isset($data['UserId'])) {
    $ride_id = $data['ride_id'];
    $ride_status = $data['ride_status'];
    $DriverId = $data['UserId'];

    try {
        $stmt = $pdo->prepare("UPDATE services SET DriverId= ?, RideStatus = ? WHERE RideId = ?");
        $stmt->execute([$DriverId, $ride_status, $ride_id]);
